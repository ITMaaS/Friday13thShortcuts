...
$shortcutName="HyperSnap 7"
#Define Start Menu Location
$StartMenuDir = Join-Path -Path $env:ALLUSERSPROFILE -ChildPath "Microsoft\Windows\Start Menu\Programs\HyperSnap 7"
#Define executable path
$ExectuablePath = "c:\program files (x86)\Hypersnap 7\Hprsnap7.exe"
...
# Declare remediation required, exit 1 to flag for remediation in Proactive Remediations
    {Write-Host "Shortcut for $shortcutName not found, start remediation"
    Exit 1
}
}
#Declare that executable isn't found, so shortcut creation isn't required.
else {Write-Host "$shortcutName Executable Not Found, no action required"
Exit 0}